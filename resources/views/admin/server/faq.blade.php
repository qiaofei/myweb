...
@section('content')
    <div class='box'>
        <div class="box-header with-border" id="box-header">
            ...
            <div class="box-tools">
                <a _rel="popup" href="{{ url('admin/server/faq/createFaq') }}" class="btn btn-primary fa fa-plus" _title="{{ '新增分平台账号' }}" _height="760" _width="1100">&nbsp;{{ '新增' }}</a>
            </div>
        </div>
        <div class="search_b" id="search_b">
            {!! Form::open( ['url'=>'/admin/server/faq']) !!}
            {{'问题归属标题'}}
                    ...
                    <table class="scroll_table table table-hover">
                        ...
                        @foreach ($faq_list as $key=>$faq)
                            <tr >
                                ...
                                <td align="center">{{ $title_list[$faq->title_id] }}</td>
                                <td align="center">{{ $faq->question }}</td>
                                <td align="center">{!! $faq->answer !!}</td>
                                <td align="center">{{ date("Y-m-d H:i:s",$faq->create_time) }}</td>
                                <td align="center">
                                    <div class="btn-group">
                                        <a _rel="popup" class="btn btn-primary" href="{{ url('/admin/server/'.$faq->id.'/editFaq')  }}" _title="{{ '编辑内容' }}" _height="760" _width="1100"><i class="fa fa-pencil fa-fw"></i>&nbsp;{{ '编辑' }}</a>
                                        <a class="btn btn-primary dropdown-toggle" data-toggle="dropdown" href="#">
                                            <span class="fa fa-caret-down"></span></a>
                                        <ul class="dropdown-menu">
                                            <li ><a _rel="confirm" href="{{ url('/admin/server/'.$faq->id.'/delFaq')  }}" _title="{{ '删除数据' }}" ><i class="fa fa-trash-o fa-fw"></i>&nbsp;{{ '删除' }}</a></li>
                                        </ul>
                                    </div>
                                </td>
                            </tr>
